...
RSE y MACH-IV cubren *k*=4 y *k*=5, el mismo rango bajo ya usado con estos instrumentos en un estudio hermano de simulación que compara SSTN contra pruebas clásicas de normalidad; HEXACO y RWAS son instrumentos nuevos incorporados específicamente para este estudio, seleccionados para extender el rango de *k* hacia valores más altos.
...
A diferencia del diseño de referencia (que fija la carga factorial en λ=0.8 y calibra solo los umbrales), aquí λ se trata como un parámetro libre de la calibración, junto con los umbrales. Esta decisión responde a un chequeo de factibilidad previo: con λ fija, el mecanismo factor-común más umbrales discretizados solo alcanza combinaciones de asimetría/curtosis platicúrticas o cercanas a la normal; dejando λ libre, el espacio alcanzable se amplía sustancialmente e incluye curtosis positiva, necesaria para reproducir el nivel más extremo del diseño (véase más abajo).
...
Se evaluaron las mismas once pruebas usadas en el estudio hermano: Shapiro-Wilk (Shapiro y Wilk, 1965), Anderson-Darling (Anderson y Darling, 1954), Lilliefors (Lilliefors, 1967), Jarque-Bera (Jarque y Bera, 1980), D'Agostino-Pearson (D'Agostino y Pearson, 1973), Cramér-von Mises (Csörgő y Faraway, 1996), Shapiro-Francia (Shapiro y Francia, 1972), χ² de Pearson (Pearson, 1900), la prueba de curtosis de Anscombe-Glynn (Anscombe y Glynn, 1983), Epps-Pulley (basada en la función característica empírica) y el SSTN (Anarat y Schwender, 2026). A diferencia del estudio hermano, aquí ninguna de las once ocupa un rol protagónico: el objeto de estudio es el efecto de *k*, *n* y la severidad de la no-normalidad sobre la potencia comparada de la batería completa, no el desempeño relativo de una prueba en particular.
...
- Pendiente: decidir si se incluye el detalle completo de las fórmulas (momentos, parametrización de umbrales `primer_umbral + cumsum(exp(log_gaps))`, transformación `plogis`/`qlogis` de λ) en el cuerpo del Método o se relega a un apéndice/material suplementario — el estudio hermano no incluye fórmulas en el cuerpo, solo prosa descriptiva.
